...
struct HealthCareCatalogView: View {
    ...
    private var groupedPackages: [String: [Package]] {
        let filtered = searchText.isEmpty ? packageManager.packages : packageManager.searchPackages(query: searchText)
        return Dictionary(grouping: filtered) { $0.department.name }
    }
